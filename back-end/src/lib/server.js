...
//    * load routes
app.use(require('../route/country-router.js'));
app.use(require('../route/place-router.js'));

// add 404 route
app.all('/api/*', (req, res, next) => res.sendStatus(404)); // eslint-disable-line
...
server.stop = () => {
  return new Promise((resolve, reject) => {
    ...
    return reject(new Error('ther server is not running'));
  });
};
